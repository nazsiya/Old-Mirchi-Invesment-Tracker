...
with st.form("add_transaction"):
    ...
    with col2:
        ...
        if selected_purpose == "Custom - Type below":
            purpose = st.text_input(
                "Enter Custom Purpose", 
                placeholder="e.g., Spice Grinder Machine, Chef Training, etc.",
                help="Type your specific investment purpose"
            )
        else:
    ...
    if submitted:
        if purpose and amount > 0 and purpose.strip():
            # Handle empty custom purpose
            if purpose == "Custom - Type below":
                st.session_state.message = ("error", "Please enter a custom purpose in the text field below")
                st.rerun()
                return
